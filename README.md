...
- Windows and Linux
- TOML config: auto-picks the sole `*t3b.conf` in `$PWD`, or `-config path`; `-config_write` drops an example; `t3b check config` audits keys (optional `-write` merges missing ones)
...
t3b check config               # validate + report keys missing vs current example
t3b check config -write        # append/insert missing keys; keeps your values
just run                       # foreground
```

After upgrading t3b, run `check config` (and `-write` if you want the new knobs written into the file). The bot still applies defaults for omitted keys at runtime; check is so Meat Bags see and edit everything the example documents.
